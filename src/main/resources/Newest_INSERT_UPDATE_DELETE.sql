...
select count(Surname) from Customers;
...
select count(Shop_Name) from Customers;

-- insert 3
INSERT INTO Phones(id, Brand, Model, Release_Date, Customers_id)
VALUES(666, 'APPLE', 'IPHONE 12 PRO MAX', '2020-10-13',  1),
(667, 'SAMSUNG', 'Galaxy S21 Ultra', '2021-01-14',  2);


-- insert 4
INSERT Phones_Details(id,Phone_Brand,Phone_Model,Color,ROM,RAM,Battary,Price,Phones_id)
VALUES(190,'APPLE','IPHONE 12 PRO MAX','grey', '128', '6', 'built-in rechargeable lithium-ion battery', 150, 666);

select count(Phone_Brand) from Phones_Details;
...
-- insert 9
INSERT Shop_Employees(id,Name,Surname,Contact_Number,Email,Address,Position,Shop_id)
...
UPDATE Phones SET Release_Date='2020-10-13' WHERE id=666;
UPDATE Phones_Details SET Color='pink' WHERE id=190;
...
UPDATE Shop_Employees SET Email='updated@example.com' WHERE id=1;
UPDATE Inventory SET Quantity=128 WHERE id=1;

-- delete 10 example
DELETE FROM Customers WHERE id= 1;
DELETE FROM Shop WHERE id=5001;
DELETE FROM Phones WHERE  id=666;
DELETE FROM Phones_Details WHERE  id=190;
...
DELETE FROM Shop_Employees WHERE id=1;
...
SELECT `id`,`Position` FROM Shop_Employees AS `id`,`Position`;
...
SELECT *  
FROM Customers
INNER JOIN Phones ON Customers.id = Phones.Customers_id
JOIN Phones_Details ON Phones.id=Phones_id
JOIN Phones_has_Shop ON Phones.id=Phones.id
JOIN Phone_Warranties ON Phone_Warranties.Phones_Details_Phones_id=Phones.id
JOIN Returns ON Returns.Customers_id=Customers.id
JOIN Shop_has_Customers ON Shop_has_Customers.Customers_id=Customers.id
JOIN Shop_Employees ON Shop_Employees.Shop_id=Shop_has_Customers.Shop_id
JOIN Service_Request ON Service_Request.Customers_id=Customers.id
...
SELECT *  
FROM Customers
LEFT JOIN Phones ON Customers.id = Phones.Customers_id;

SELECT * FROM Phones_Details
JOIN Phone_Warranties ON Phone_Warranties.Phones_Details_Phones_id=Phones_Details.Phones_id;

SELECT * FROM Phones
RIGHT JOIN Phone_Warranties ON Phone_Warranties.Phones_Details_Phones_id=Phones.id;
